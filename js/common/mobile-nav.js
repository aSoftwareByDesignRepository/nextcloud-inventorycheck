/**
 * InventoryCheck mobile navigation: in-page Menu button + slide-in drawer.
 * On mobile open, nav + backdrop portal to document.body so #content /
 * #inventorycheck-app overflow:hidden cannot clip the drawer (arbeitszeitcheck#33
 * / Atlas ATLAS_MOBILE_NAV_CONTRACT). Core #app-navigation-toggle alone is not enough.
 *
...
(function () {
	...
	const APP_ID = 'inventorycheck';
	const DESKTOP_MQ = '(min-width: 1024px)';
	const BACKDROP_ID = 'ivc-nav-backdrop';
	...
	function init() {
		...
		const toggle = document.getElementById('ivc-nav-toggle');
		const nav = document.getElementById('app-navigation');
		const shellRoot = document.getElementById('inventorycheck-app');
		...
		document.body.classList.add('ivc-has-nav');
		...
		if (!backdrop) {
			...
			backdrop.className = 'ivc-nav-backdrop';
			backdrop.hidden = true;
			shellRoot.insertBefore(backdrop, appContent);
		}

		const shell = document.querySelector('#app-content.iv-app > #app-content-wrapper.iv-shell')
			|| document.querySelector('#app-content.iv-app > .iv-shell');
		...
		function setOpen(open) {
			...
			nav.classList.toggle('ivc-nav--open', open);
			toggle.setAttribute('aria-expanded', open ? 'true' : 'false');
			toggle.setAttribute('aria-label', open ? closeLabel : openLabel);
			document.body.classList.toggle('ivc-nav-open', open);
			document.body.classList.remove('snapjs-left');
			...
			if (open) {
				...
				trapHandler = function (event) {
					if (!nav.classList.contains('ivc-nav--open') || event.key !== 'Tab') {
						return;
					}
		...
		toggle.addEventListener('click', (event) => {
			...
			setOpen(!nav.classList.contains('ivc-nav--open'));
		});
		...
		document.addEventListener('keydown', (event) => {
			if (event.key === 'Escape' && nav.classList.contains('ivc-nav--open')) {
				setOpen(false);
				toggle.focus();
			}
		});
		...
		function onViewportChange() {
			...
			if (desktopMq.matches) {
				...
				restoreDomOrder();
				nav.removeAttribute('aria-hidden');
			} else if (!nav.classList.contains('ivc-nav--open')) {
				restoreDomOrder();
	...
	window.InventoryCheckMobileNav = {
		init: boot,
		close() {
