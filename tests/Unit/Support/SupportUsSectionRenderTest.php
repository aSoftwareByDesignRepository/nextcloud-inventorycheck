...
use OCA\InventoryCheck\Support\SupportUsLinks;
...
final class SupportUsSectionRenderTest extends TestCase {
	...
	public function testRenderEscapesDisplayNameAndOmitsMobileWithoutFlag(): void {
		$html = $this->renderSection(
			new SupportUsLinks('InventoryCheck', false, null),
			'en'
		);
		...
		self::assertStringContainsString(rawurlencode('InventoryCheck: partner / care retainer'), $html);
	...
	public function testRenderIncludesMobileLicenseWhenConfigured(): void {
		$html = $this->renderSection(
			new SupportUsLinks(
				'InventoryCheck',
				true,
				'/apps/inventorycheck/admin/license'
			),
			'de'
		);
		...
		self::assertStringContainsString('href="/apps/inventorycheck/admin/license"', $html);
		self::assertStringContainsString(rawurlencode('InventoryCheck: Partner / Care Retainer'), $html);
