...
final class AppFeedbackLinksTest extends TestCase
{
	...
	{
		$links = new AppFeedbackLinks('inventorycheck', 'InventoryCheck', '1.2.3');
		$en = $links->problemMailto(['pageUrl' => '/apps/inventorycheck/today'], 'en');
		$de = $links->problemMailto(['pageUrl' => '/apps/inventorycheck/today'], 'de');
		self::assertStringStartsWith('mailto:dev@software-by-design.de?subject=', $en);
		self::assertStringContainsString(rawurlencode('InventoryCheck: Problem report'), $en);
		self::assertStringContainsString(rawurlencode('InventoryCheck: Fehlermeldung'), $de);
	...
	{
		$links = new AppFeedbackLinks('inventorycheck', 'InventoryCheck');
		$href = $links->ideaMailto([], 'de');
		self::assertStringContainsString(rawurlencode('InventoryCheck: Feedback'), $href);
	}
	...
	public function testGithubIssuesUrlIsPublicVendorRepo(): void
	{
		$links = new AppFeedbackLinks('inventorycheck', 'InventoryCheck');
		self::assertSame(
			'https://github.com/aSoftwareByDesignRepository/nextcloud-inventorycheck/issues',
	...
	{
		$links = new AppFeedbackLinks('inventorycheck', 'InventoryCheck');
		$clean = $links->sanitizePageUrl('https://cloud.example/apps/inventorycheck/x?token=abc&keep=1&password=no');
		...
		$keep = $links->sanitizePageUrl('https://cloud.example/apps/inventorycheck/list?keep=1&token=abc');
	...
	{
		$links = new AppFeedbackLinks('inventorycheck', 'InventoryCheck');
		$noisy = '/apps/inventorycheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis';
		self::assertSame('/apps/inventorycheck/', $links->sanitizePageUrl($noisy));
	}

	public function testMailtoBodyOmitsIdentityAndRejectsUnsafeErrorCodes(): void
	{
		$links = new AppFeedbackLinks('inventorycheck', 'InventoryCheck', '9.9.9');
		$href = $links->problemMailto([
			'errorCode' => 'CONFLICT',
			'pageUrl' => '/apps/inventorycheck/',
		], 'en');
		...
		self::assertStringContainsString('App id: inventorycheck', $body);
	...
	public function testRejectsUnsafeDisplayName(): void
	{
		...
		new AppFeedbackLinks('inventorycheck', "Evil\r\nBcc: attacker@example.com");
	}

	public function testForLocalePayload(): void
	{
		$links = new AppFeedbackLinks('inventorycheck', 'InventoryCheck', '1.0.0');
		$payload = $links->forLocale('de', ['pageUrl' => '/apps/inventorycheck/']);
		...
		self::assertSame('inventorycheck', $payload['appId']);
	}
}
