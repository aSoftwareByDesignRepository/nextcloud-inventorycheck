#!/usr/bin/env node
/**
 * Behavioural tests for InventoryCheck app-feedback.js (mailto + toast hook).
 *
...
function boot(extra) {
	...
	const config = {
		...
		appDisplayName: 'InventoryCheck',
		appVersion: '1.0.0',
		feedbackEmail: 'dev@software-by-design.de',
		githubIssuesUrl: 'https://github.com/aSoftwareByDesignRepository/nextcloud-inventorycheck/issues',
		cssPrefix: 'iv',
		...(extra || {}),
	};
	const cfgEl = {
	...
	nodes.set('iv-app-feedback-config', cfgEl);
	...
	const sandbox = {
		...
		location: {
			...
			pathname: '/apps/inventorycheck/today',
			search: '?token=secret&keep=1',
			href: 'https://cloud.example/apps/inventorycheck/today?token=secret&keep=1',
		},
...
function main() {
	...
	const clean = sandbox.SbdAppFeedback.sanitizePageUrl(
		'https://cloud.example/apps/inventorycheck/x?token=abc&keep=1&password=no'
	);
	...
	process.stdout.write('app-feedback.test.js OK (inventorycheck)\n');
}
