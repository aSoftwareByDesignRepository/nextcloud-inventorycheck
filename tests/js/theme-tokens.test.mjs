...
test('tokens map to Nextcloud theme variables with scrim/touch/qr canvas', () => {
	...
	assert.ok(tokens.includes('--space-1: var(--iv-space-1)'))
	assert.ok(tokens.includes('prefers-contrast: more'))
})
...
test('app.css imports tokens and uses theme-safe surfaces', () => {
	...
	assert.ok(appCss.includes('forced-colors: active'))
	assert.ok(appCss.includes('prefers-contrast: more'))
