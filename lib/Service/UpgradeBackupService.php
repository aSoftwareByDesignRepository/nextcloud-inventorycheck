...
/**
 * Snapshot and restore InventoryCheck data before app updates.
 *
...
class UpgradeBackupService
{
	...
	{
		return $this->runExclusive('InventoryCheck upgrade backup', function () use ($reason, $rotate, $rotationExcludeIds): array {
	...
	private function doCreateSnapshot(string $reason, bool $rotate, array $rotationExcludeIds): array
	{
		...
		if (!$this->hasDataToBackup()) {
			throw new UpgradeBackupException('No InventoryCheck tables exist yet; nothing to back up.');
		}
		...
		try {
			...
			if ($backedUpTables === []) {
				throw new UpgradeBackupException('No InventoryCheck tables exist on this instance; nothing to back up.');
			}
		...
		$this->logger->info('InventoryCheck: created pre-update backup snapshot', [
	...
	public function restoreSnapshot(string $snapshotId, bool $createSafetySnapshot = true): void
	{
		...
		$this->runExclusive('InventoryCheck upgrade restore', function () use ($snapshotId, $folder, $tableManifest, $createSafetySnapshot): void {
	...
	private function doRestoreSnapshot(
		...
		try {
			...
			$this->db->rollBack();
			throw new UpgradeBackupException('InventoryCheck restore failed; database changes were rolled back.', 0, $e);
		} finally {
			if ($fkChecksDisabled) {
		...
		try {
			...
			throw new UpgradeBackupException(
				'InventoryCheck database was restored but app config or app data folders failed. '
		...
		$this->logger->warning('InventoryCheck: restored data from upgrade backup snapshot', [
			'app' => UpgradeBackupCatalog::APP_ID,
	...
	private function getAppDataRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('InventoryCheck app data root is not a folder.');
		}
	...
	private function getBackupRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('InventoryCheck upgrade backup root is not a folder.');
		}
	...
	private function runExclusive(string $label, callable $callback)
	{
		try {
			...
			throw new UpgradeBackupException(
				'Another InventoryCheck backup or restore is already in progress.',
				0,
				$e,
			);
		}
