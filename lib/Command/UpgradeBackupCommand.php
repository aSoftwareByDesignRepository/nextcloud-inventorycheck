...
/**
 * List, create, or restore InventoryCheck upgrade backups (CLI operator interface).
 *
...
{
	private const APP_LABEL = 'InventoryCheck';
	...
	{
		$this
			->setName('inventorycheck:upgrade-backup')
			->setDescription('Manage InventoryCheck snapshots taken before app updates.')
			->setHelp(
				<<<'HELP'
...
<info>create</info> — manual snapshot before risky changes:
  <comment>occ inventorycheck:upgrade-backup create --reason="before manual fix"</comment>

<info>restore</info> — destructive; requires <comment>--force</comment> and snapshot id:
  <comment>occ inventorycheck:upgrade-backup restore --latest --force</comment>
  <comment>occ inventorycheck:upgrade-backup restore --id=YYYYMMDDTHHMMSSZ-hex --force</comment>
...
HELP
			...
			->addOption('force', null, InputOption::VALUE_NONE, 'Required for restore — overwrites current InventoryCheck data')
	...
	private function runList(SymfonyStyle $io): int
	{
		...
		if ($snapshots === []) {
			...
			$io->note('Create one manually: occ inventorycheck:upgrade-backup create --reason="before change"');
	...
	private function runRestore(InputInterface $input, SymfonyStyle $io): int
	{
		...
		if (!$input->getOption('force')) {
			$io->error([
				'Restore overwrites all current InventoryCheck database rows, app config, and configured app-data folders.',
		...
		if ($snapshotId === '') {
			$io->error([
				...
				'  occ inventorycheck:upgrade-backup restore --latest --force',
				'  occ inventorycheck:upgrade-backup restore --id=<snapshot-id> --force',
				'Run `occ inventorycheck:upgrade-backup list` to see snapshot ids.',
			]);
			return Command::FAILURE;
		}
