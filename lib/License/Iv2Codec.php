...
namespace OCA\InventoryCheck\License;

use OCA\InventoryCheck\Config\VendorPublicKey;

/**
 * IV2 wire: `IV2.<payload_b64url>.<signature_b64url>`, Ed25519 over canonical JSON.
 *
 * Canonical key order: v, product, customerId, issuedAt, validUntil,
 * mobileSeats, scanDevices, bundle (only when true).
 *
 * Constraint: mobileSeats + scanDevices > 0.
 * Dates never block acceptance; expiry evaluated at gate time.
 */
final class Iv2Codec
{
	public const FORMAT = 'IV2';
	public const VERSION = 2;
	public const PRODUCT = 'inventorycheck';
	...
	public static function validatePayloadFields(array $payload): bool
	{
		...
		$devices = $payload['scanDevices'] ?? null;
		if (!is_int($seats) || $seats < 0 || $seats > 10000) {
			return false;
		}
		if (!is_int($devices) || $devices < 0 || $devices > 1000) {
			return false;
		}
		if ($seats + $devices <= 0) {
			return false;
		}
		if (array_key_exists('bundle', $payload)) {
			if ($payload['bundle'] !== true) {
	...
	public static function canonicalJson(array $payload): string
	{
		$ordered = [
			...
			'scanDevices' => (int)$payload['scanDevices'],
		];
		...
		if ($json === false) {
			throw new \RuntimeException('IV2 canonical JSON encode failed.');
		}
		return $json;
	}
